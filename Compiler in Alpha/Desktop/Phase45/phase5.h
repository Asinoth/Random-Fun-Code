...
{
    std::string s;
    // s = std::string(namedLibfuncs[avm->data.libfuncVal]); //VGAZEI THEMA STO COMPILE, DEN KSERW PWS THA FTIAXEI....Pfff
    return strdup((char *)s.c_str());
}
...
void libfunc_strtonum() {
    ...
    else
    {
        avm_memcellclear(&retval);
        retval.type = number_m;
        retval.data.numVal = std::atoi(avm_getactual(0)->data.strVal);
    }
}
void libfunc_sqrt() {
    unsigned n = avm_totalactuals();
...
void execute_call(instruction *i)
{
    ...
    case userfunc_m:
    {
        ...
        assert(code[pc].vmop == funcenter_v);
        break;
    }
